...
async def list_wallets(
    ...
    # Only LIVE owners resolve a name; a soft-deleted tenant's wallet is history, not a
    # management surface, so it is dropped from the listing below.
    un = {u: n for u, n in (await db.execute(select(User.id, User.name).where(User.id.in_(uids), User.deleted_at.is_(None)))).all()} if uids else {}
    gn = {g: n for g, n in (await db.execute(select(Project.id, Project.name).where(Project.id.in_(gids), Project.deleted_at.is_(None)))).all()} if gids else {}
    on = {o: n for o, n in (await db.execute(select(Organization.id, Organization.name).where(Organization.id.in_(oids), Organization.deleted_at.is_(None)))).all()} if oids else {}
    ...
    out = []
    for w in rows:
        # user/group/org wallets whose owner is soft-deleted disappear with the owner; wallet
        # types without an owner row (e.g. system) always list.
        if w.owner_type in ("user", "group", "org") and _own(w) is None:
            continue
        wv = _wallet_view(w)
