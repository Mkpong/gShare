...
class GShareSessionCRD:
    ...
    async def _resolve_idle_timeout_sec(self, spec: dict[str, Any]) -> int | None:
        ...
        if pol is None:
            return None
        # CPU sessions have their own reaper windows (limits.cpu_session_*); the GPU fields must
        # not leak onto them — a GPU idle/max cap stamped on a CPU session reaped it out of nowhere.
        if spec.get("resource_class") == "cpu":
            val = (pol.limits or {}).get("cpu_session_idle_timeout_sec")
        else:
            val = pol.idle_timeout
        if val is None:
            return None
    ...
    async def _resolve_max_runtime_sec(self, spec: dict[str, Any]) -> int | None:
        ...
        if pol is None:
            return None
        # Same class split as the idle window: CPU sessions read limits.cpu_session_max_runtime_min.
        if spec.get("resource_class") == "cpu":
            val = (pol.limits or {}).get("cpu_session_max_runtime_min")
        else:
            val = pol.max_runtime
