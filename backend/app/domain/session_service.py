...
class SessionService:
    ...
    async def terminate(self, session_id: str, *, forced: bool = False, reason: str = "user_stopped"):
        ...
        if billable:
            # Only a session that was on the clock owes a final partial minute. A paused one was
            # trued up at stop(); recomputing to now() would bill the whole paused gap. A retry
            # that finds `terminating` cannot know the prior state, so it also skips — the worker
            # billed every full minute while it ran, so at most one partial minute is forgone.
            await self.credit.settle(
                sess, key=f"settle:{sess.id}", final_consume=(prior_status == "running")
            )
        sess = await self._get(session_id)
