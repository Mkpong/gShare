...
@pytest.mark.asyncio
async def test_settle_paused_session_does_not_bill_the_paused_gap(db):
    """Terminating from paused must not recompute owed to now() (the paused hours).

    Regression: settle's final consume ran unconditionally from started_at to now(), so a session
    paused for hours was billed the whole gap at terminate — and when that exceeded the balance,
    the unclamped debit violated wallet_sigma, the terminate 500'd, and the session stuck in
    `terminating`.
    """
    wallet = await _make_wallet(db, balance="50")
    ...
    after = await _reload(db, wallet.id)
    assert after.balance == Decimal("50.00")


@pytest.mark.asyncio
async def test_settle_final_consume_clamps_at_balance(db):
    """Even when a final consume is owed, the debit stops at 0 instead of violating the constraint."""
    wallet = await _make_wallet(db, balance="10")
    started = datetime.now(UTC) - timedelta(hours=1)
    sess = Session(
        id=ids.new("session"), owner_user_id=ids.new("user"), cluster_id=ids.new("cluster"),
        offering_id=ids.new("offering"), image_id=ids.new("image"), resource_class="gpu",
