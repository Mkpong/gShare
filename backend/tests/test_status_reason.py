...
@pytest.mark.asyncio
async def test_reaper_terminating_message_sets_max_runtime_reason(db):
    """The reaper reports Terminating("max-runtime-exceeded") then the finalizer reports a
    message-less Terminated; the reason from the first event must survive onto the session."""
    sess = await _running_session(db)
    sync = StatusSync(db)
    await sync.on_status(sess.id, OperatorStatusEvent(
        phase="terminating", ts=datetime.now(UTC), message="max-runtime-exceeded"))
    await sync.on_status(sess.id, OperatorStatusEvent(phase="terminated", ts=datetime.now(UTC)))
