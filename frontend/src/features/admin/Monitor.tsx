...
export function AdminMonitor() {
  ...
  const queueColumns: Column<QueueRow>[] = useMemo(
    () => [
      { key: 'position', header: '#', sortBy: (q) => q.position ?? 0, align: 'center', headerClassName: 'w-16 whitespace-nowrap', render: (q) => <b>{q.position ?? '-'}</b> },
      {
        key: 'session_id',
        header: t('admin.monitor.colSession'),
        headerClassName: 'w-full',
        sortBy: (q) => q.session_name ?? q.session_id ?? '',
        render: (q) => (
          <b className="truncate max-w-[220px] inline-block align-bottom" title={q.session_id}>
      ...
      {
        key: 'owner',
        header: t('admin.monitor.colOwner'),
        align: 'center',
        headerClassName: 'whitespace-nowrap px-6',
        cellClassName: 'px-6',
        sortBy: (q) => q.owner_name ?? '',
        render: (q) => <span className="text-muted text-xs whitespace-nowrap">{q.owner_name ?? '-'}</span>,
      },
      { key: 'priority', header: t('admin.monitor.colPriority'), sortBy: (q) => q.priority ?? 0, align: 'center', headerClassName: 'whitespace-nowrap px-6', cellClassName: 'px-6', render: (q) => <b>{q.priority}</b> },
      { key: 'enqueued_at', header: t('admin.monitor.colEnqueued'), sortBy: (q) => (q.enqueued_at ? new Date(q.enqueued_at).getTime() : 0), align: 'center', headerClassName: 'whitespace-nowrap px-6', cellClassName: 'px-6 whitespace-nowrap', render: (q) => <Timestamp value={q.enqueued_at} className="text-muted" /> },
      {
        key: 'actions',
        header: t('admin.monitor.colActions'),
        align: 'right',
