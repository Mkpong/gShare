...
The image catalog is what the session wizard offers. An image that is not here (or not
...
|---|---|
| **Name** | What users see |
| **Kind** | Catalog image, or a user's own |
...
Two tabs: the **catalog** itself, and **builds** — every console-driven build with its state
...
With `api.seedSessionImages` (the default) the catalog seeds `boanlab/gshare-session` for the
...
`incompatible_image` before the session starts, which is the behavior you want — the alternative
is a container that starts and then cannot see the GPU.

## Keeping the catalog small
